...
Warehouse data sources are are suitable for storing files in all cases, except where the files need to be accessed by a webapp. In this case, files should be stored in an app data source.
...
You can upload files to Amazon S3 from your local computer or from your home 

*   You should follow information governance procedures when uploading data to Amazon S3.

### Amazon S3 Console

You can use the Amazon S3 Console to upload files from your local computer (e.g., personal or shared storage on DOM1 or Quantum) only.
...
4.  Select the folder you want to upload files to.
5.  Select 'Upload'.
6.  Select 'Add files' or drag and drop the files you want to upload.
7.  Select 'Upload' -- you do not need to complete steps 2, 3 or 4.
...
You can find out more about how to use these functions on [GitHub](https://github.com/moj-analytical-services/s3tools) or by using the help operator in RStudio (e.g., `?s3tools::write_file_to_s3`).
...
```{python install-boto3, eval=FALSE}
pip install boto3
```

To upload a file to Amazon S3, you should use the following code:

```{python python-upload-s3, eval=FALSE}
import boto3
...
You can use the Amazon S3 Console to download files to your local computer (e.g., personal or shared storage on DOM1 or Quantum) only.
...
5.  Select 'Download' or 'Download as' as appropriate.
...
You can find out more about how to use these functions on [GitHub](https://github.com/moj-analytical-services/s3tools) or by using the help operator in RStudio (e.g., `?s3tools::download_file_from_s3`).
...
You can use any of the `pandas` read functions (e.g., `read_csv` or `read_json`) to download data directly from Amazon S3. This requires that you have installed the `s3fs` package. To install the `s3fs` package, run the following code in a terminal:

```{python install-s3fs, eval=FALSE}
pip install s3fs
```

As an example, to read a CSV, you should run the following code:

```{python read-pandas, eval=FALSE}
import pandas as pd
