...
- Analysts themselves are responsible for the code they write in these tools, and the Platform team is not responsible for assisting with problems with this code.  As a rule of thumb, if the problem you're experiencing would also occur with R Studio or Python installed on a standalone computer, then the platform team cannot offer support.
- If you've read through this user guidance and are still stuck, the best place to go for support is the [analytical-platform](https://asdslack.slack.com/messages/C4PF7QAJZ/#) Slack channel for issues with the platform, or [the R channel]and (https://asdslack.slack.com/messages/C1PUCG719/#)[python channel](https://asdslack.slack.com/messages/C1Q09V86S/#) to get support from peers in your code.
...
For only those users who cannot access slack or Github (i.e. some non DOM1 users) there is a direct email address which can be used: robin.linacre@digital.justice.gov.uk - but this will be monitored for emails from non DOM1 users.
