...
*Public repos* means visible to the world. A repo should contain no passwords/secrets and no data (apart from small reference tables) - these are particulary important for public repos.

*Private repos* are always internal to the moj-analytical-services GitHub organisation. The repo's Owners and Admins can control which people / teams can see the repo on GitHub by going to **_Settings_ > _Collaborators & Teams_** and adding teams to read/write/admin access groups. To maximise collaboration _within_ the organisation but still keep code that shouldn't be in the public domain (e.g. unpublished commentary on a `.Rmd` file) hidden from anyone _outside_ the organisation, you can add the group `everyone` here. This also makes your code [searchable.](https://moj-analytical-services.github.io/platform_user_guidance/using-github-with-the-platform.html#other-tips-and-tricks-work-in-progress)

#### Choosing public or private

As an organization we aspire to use public repos by default. There are a host of [benefits of coding in the open](https://gds.blog.gov.uk/2017/09/04/the-benefits-of-coding-in-the-open/). With research and analysis it builds trust with the public for the state to be transparent. And reproducible methods allows others to test and build on the work. However it requires more discipline to avoid mistakes like checking in secrets and sensitive information, so tends to require more experienced developers. And you need to be conscious of any political sensitivities related to the topics.

Private repos can be necessary, for example when it reveals a sensitive policy change that is not yet announced.

If in doubt, consult someone more senior, or the AP team.

### Private R Packages for Reproducible Analysis

When a repo is private you need to use a 'token' called a PAT to access it from R. If you don't use the token then you get: `Error: HTTP error 404. Not Found`. (GitHub doesn't even acknowledge the existence of the repo, to avoid speculative searching for private repositories.)
...
A Personal Access Token (PAT) is provided by GitHub through each user's profile, and in fact a user can generate a number of tokens that grant various different levels of permission. **The PAT is a password, and should be protected as such: only generate a PAT that has the minimum permissions you need for the job.** To generate your PAT and use it in `R`,
  ...
  3. Copy your PAT to the clipboard, for immediate use - see the next section.
  
     Note: GitHub won't show this PAT to you ever again BUT you shouldn't save it in a file or anything. If you were to have passwords in plain text lying around your hard disk then it becomes a security hazard. If you need a PAT in future (eg you wiped your .simply create a new one, and invalidate the previous one.
...
Conveniently, most [`r-lib`-style](https://github.com/r-lib) packages (those built by Hadley Wickham and friends--for most people a backbone of R infrastructure) including `remotes` and `devtools`, will look for a PAT in the system environment by default, under the name `GITHUB_PAT`, so no modifications need be made to any project code once a user has registered a PAT in this location.
