...
In either case, they should contain **no passwords/secrets and no data** (apart from small reference tables) - this is particulary important for public repos.
Think carefully about what a determined outsider could infer from your code, and whether you'd be comfortable telling them yourself.

A *Public repo* means visible to the world outside the organisation. Again, here it is especially crucial that no passwords, secrets or personal tokens are included in your code, whether you think they're visible or buried deep. 
Remember that GitHub shows the full history of *everything that has ever been in your repo*, however fleeting, so even if you 'just include it for now' a password or secret token will **always be available**. 
...
When a repo that forms a package for reproduceable analysis is private, a 'token' called a PAT is required to access it from R, which acts much likea limited-access password to your GitHub account.
This prevents unauthorised access to the organisation's repos.
Without the right token, trying to install a Private package return the error: `Error: HTTP error 404. Not Found`.
(GitHub doesn't even acknowledge the existence of the repo, to avoid speculative searching for private repositories.)

### Generating a PAT

A Personal Access Token (PAT) is provided by GitHub through each user's profile, and in fact a user can generate a number of tokens that grant various different levels of permission.
**The PAT is a password, and should be protected as such: only generate a PAT that has the minimum permissions you need for the job.**
To generate your PAT and use it in `R`,
  ...
  **IMPORTANT: Unless you have specific reasons to need more control, just select the first group, `repo`--granting other rights can be dangerous if your PAT falls into the wrong hands, allowing someone else to irrevocably delete your repos, read and change your user profile settings, or even access billing information.**
  ![](images/pat/pat-scope.png)
  
  3. Copy your PAT to the clipboard, for immediate use - see the next section.
  
     Note: GitHub won't show this PAT to you again BUT do **not** save it in a file.
     Leaving passwords in plain text lying around your hard disk is a security hazard.
     If you need a PAT in future (e.g. you wiped your current one or need different access rights), simply create a new one, and invalidate the previous one.
     You can revoke a PAT at any time, so that it no longer works for anyone with access to it.
