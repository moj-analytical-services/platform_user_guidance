# Using Github with the platform{#github}

Before you can use Github with R Studio or Jupyter, you need to connect them together by creating an 'ssh key'.  Full guidance is [here](https://moj-analytical-services.github.io/platform_user_guidance/getting-started.html#configuring-git-and-github-for-use-on-the-analytical-platform)
...
* Note you can add one or more teams to a repository, each with different permissions. For example, your team could have write privileges, but the ‘everyone’ team could be read only.,
...
The majority of repos in the DASD GitHub organisation are **Private**: this keeps visibility, access and editing rights controlled to particular groups within the MoJ network and is sensible in most cases. Owners and Admins of a repository can control this visibility by going to **_Settings_ > _Collaborators & Teams_** and adding teams to read/write/admin access groups. To maximise collaboration _within_ the organsiation but still keep code that shouldn't be in the public domain (e.g. unpublished commentary on a `.Rmd` file) hidden from anyone _outside_ the organisation, you can add the group `everyone` here. This also makes your code [searchable.](https://moj-analytical-services.github.io/platform_user_guidance/using-github-with-the-platform.html#other-tips-and-tricks-work-in-progress)

### Private R Packages for Reproducible Analysis

For the most part, this private group-based access works well and seamlessly. 
Where it can rub is in building **R Packages** for high-quality reproducable analysis either across small teams or across the organisation.
The AP team have built a number of _generic_ tools that are non-sensitive in nature (e.g. [`s3tools`](github.com/moj-analytical-services/s3tools) and [`dbtools`](github.com/moj-analytical-services/dbtools)) and as such are in fact **Public** repos that anyone, inside or outside the organisation, can view, access, and install to their library.
If an internal package is being used for a specific project, RAP or other analytical work, the chances are that it will not be appropriate to make these codebases public, as some combination of the structure, code and comments can be revealing, however [sensibly the data are stored](https://moj-analytical-services.github.io/platform_user_guidance/working-with-big-datasets-and-sharing-them-accessing-data-in-amazon-s3.html).
However, when another user tries to install a _Private_ package in RStudio using [`remotes`](https://remotes.r-lib.org/) or (less commonly, now) `devtools`, _regardless_ of how the access groups are organised on GitHub, they will throw the infuriating error `Error: HTTP error 404. Not Found`. 

This is a protection mechanism.
The `R` environment isn't party to all of the access groups on GitHub, so it doesn't know who should have access to which repositories; for anything Private it returns this blanket `404` to prevent speculative searching for private repositories--the names alone could be revealing!
Fortunately, by providing `R` with a 'token' called a PAT, it can inherit access to all of the knowledge and access rights to the repositories available on your GitHub account.

### Generating a PAT

A Personal Access Token (PAT) is provided by GitHub through each user's profile, and in fact a user can generate a number of tokens that grant various different levels of permission. **The PAT is a password, and should be protected as such: only generate a PAT that has the minimum permissions you need for the job.** To generate your PAT and use it in `R`,

  1. Navigate to **_Settings > Developer Settings > Personal Access Tokens_**
  ![](images/pat/pat-settings.png)
  
  2. Select **_Generate new token_** and select the **scope** of access you want to grant using this token. **IMPORTANT: Unless you have specific reasons to need more control, just select the first group, `repo`--granting other rights can be dangerous if your PAT falls into the wrong hands, allowing someone else to irrevocably delete your repos, read and change your user profile settings, or even access billing information.**
  ![](images/pat/pat-scope.png)
  
  3. Copy your PAT into a `.txt` file that you can keep somewhere securely. You won't be able to come back to this page to see this PAT again, and will have to generate a new one if you lose track of it.
  ![](images/pat/pat-copy.png)
  
  ### Using a PAT to Authenticate in R/RStudio

A safe, fast, and reliable method is to store the PAT in the so-called **system environment**.
This is an area where basic, 'long-term' variables are kept, like your Operating System details, that do not change, even as you switch between analyses, sessions or projects.
Conveniently, most [`r-lib`-style](https://github.com/r-lib) packages (those built by the inimitable Hadley Wickham and friends--for most people a backbone of R infrastructure) including `remotes` and `devtools`, will look for a PAT in the system environment by default, under the name `GITHUB_PAT`, so no modifications need be made to any project code once a user has registered a PAT in this location.
This is the approach we'll make use of:

  1. Set the system environment variable `GITHUB_PAT` by pasting in your PAT Token into the below:
  ```{r, eval = FALSE}
  Sys.setenv("GITHUB_PAT" = "ax451...8838b1")
  ```
  
  2. Any call to `remotes::install_github` will use it without having to be asked!
  ```{r, eval = FALSE}
  remotes::install_github("moj-analytical-services/[private-package]@v1.6")
  ```
This approach means that install lines like (2) above can be freely saved in shared code without compromising GitHub security.

### Advanced 
The way `remotes::install_github` is through the parameter `auth_token`, to which the PAT can be supplied manually as a string if desired. The default is a call to `auth_token = github_pat()` which simply searches for a PAT in the system environment as above.
However, this approach leaves users **liable to leaving the PAT lying around in code**. As discussed above this is **not a good idea,** not to mention the fact that it requires the PAT to be copied-and-pasted a number of times across different projects and analyses. 

### A Note on Impermanence
Because of the pod system on which the Platform works, system environment variables are _not_ always as persisent as hoped. It's worth putting the `setenv` call above (1) into an `.RProfile` or other file **that you do not upload to GitHub** to give easy, but private access. Keep a copy of your PAT on your local machine. You can use `Sys.getenv("GITHUB_PAT")` to check whether a PAT is loaded.
