...
preview that webapp before you deploy it. The platform provides a special set of
endpoints under `/_tunnnel_/<portNumber>/` that route to exposed HTTP ports on
the localhost. There are a few things you need to keep in mind:

* `<portNumber>` can only be one from a limited rage. By default you can only route
  to ports 8050 and 4040-4050 inclusive. You should make your webapp listen on
  one of those.
...
dependencies by adding these to either `requirements.txt` & `pip` or
`environment.yaml` & `conda`.

### Example code
Save the dash hello world to a new python file called `app.py`:
...
The `Dash` class must be instantiated with a `url_base_pathname`. This should
always be `/_tunnel_/8050/` e.g.
`app = dash.Dash('app', server=server, url_base_pathname='/_tunnel_/8050/')`.

When you run the server it must be bound to a `0.0.0.0` e.g. `app.run_server(host='0.0.0.0')`
