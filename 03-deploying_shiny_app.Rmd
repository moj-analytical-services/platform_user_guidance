...
First `cd` into the directory containing the Dockerfile
`docker build . -t your_name_here`    (your_name_here is the tag you want to give to the image)

By building the image, it will test the installation of of the packrat dependencies into an identical Linux environment to what's on the platform.
...
First, change the logging settings to make the error logs more verbose:
...
Finally start the shiny server:
