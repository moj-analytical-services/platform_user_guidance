...
The deployment starts immediately and will take between 5 minutes and an hour, depending on how many R packages you've used. You can track progress and see any errors on AP's Concourse:
...
1. In the main nav, click "Discover"
2. Click on the dropdown saying "cloudtrail-*" and change it to "logstash-apps-alpha-*". Now you should see all the log messages for all apps, which you can search, but you probably want to filter as follows.
3. Click "Add a filter+ ". Fill in the form:
    * Field: `app_name`
	* Operator: "is"
	* Value: your app name, appended with `-webapp` e.g. `prison-network-app-webapp`
   Now: Save.
4. On the list of fields, click "message" then the "add" button which appears. This now displays the log messages in the main pane. The only problem is that within each timestamped batch the order of the messages is mixed up.
