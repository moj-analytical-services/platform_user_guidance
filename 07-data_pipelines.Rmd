...
The steps to setting up an airflow pipeline given below. It may appear like a lot of steps but it is broken down into small sections to avoid confusion.
...
Orange - MoJ-analytics airflow

Note: You cannot skip straight to the MoJ-analytics airflow section. You will still need to do all the steps in the local airflow section (other than test in the sandbox if you are confident it will work).


### Running DAG in local airflow
...
a. The `Dockerfile` - contains commands to set-up a Docker image that does your data processing. This is what concourse will build to create your data pipeline.
    ...
    Here's an example python one in Python. 
    ...
    # load the files we will need to run
    COPY r_files/ r_files/

    ##run the task
    ENTRYPOINT Rscript r_files/main.R
...
b. <a name="deploy.json"></a>`deploy.json` - This is for Concourse to build the repo into a Docker image
    ...
    The version number does nothing, it is not related to your release version. The `role_name` needs to be:
...
If you are using python you will need a `requirements.txt` file. If you are using R you will need an `environment.yml`, yaml file.
    ...
    **You can find the list of locally installed packages and their version numbers by typing `pip freeze` into your terminal.**
    ...
    `environment.yml`
    See the section of the guidance [here](https://moj-analytical-services.github.io/platform_user_guidance/conda-package-management.html) on how to create the `environment.yml`.
...
A local DAG script will also be required. This is what your local airflow will read and run. But it is not what the test will build from. It is very similar to the main DAG script. More on that later.
...
* if you want to run several pods in one script you must define their order as one final line in the DAG. For example, `task_1 >> task_2 >> task_3`.
...
### Running DAG in MoJ-Analytics airflow
