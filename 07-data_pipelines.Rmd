...
The airflow-dags repo uses the "Fork & Pull" model of GitHub workflow, so you will need to **fork** the airflow-dags repo to give yourself a personal copy of it (e.g. `github.com/joebloggs/airflow-dags`). The idea is that you clone and push your commits to your own fork of the airflow-dags repo. Then you create a Pull Request to the central airflow-dags repo, which will be reviewed by admins and merged. You should also locally add the central repo as a remote, and pull from there to keep up to date. Here's how to do it in detail: [Github’s Fork & Pull Workflow for Git Beginners](https://reflectoring.io/github-fork-and-pull/)
