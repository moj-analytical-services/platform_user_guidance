...
* **Warehouse data source** - This is for when data is accessed by code **run by users**, such as when you are working in **R Studio** or **Jupyter**, including when you are **running web apps from within R Studio / Jupyter**. These data sources can be created by users themselves and shared with colleagues.

* **App data source** - This is for when data is accessed by code **run by the platform** - a **deployed web app** or an **Airflow job**. Users cannot create these or add user permissions - these must be [requested](#create-webapp). Data is uploaded to them either by the app developer(s) or the Data Engineering team.

If you're building a web app then typically you request permissions for both your user and your app to access the app data source. This allows you to run the app from within R Studio or JupyterLab using the app data source, the same as the deployed version.

The Data Engineering team also have some buckets, which are not shown on the Control Panel. They are used for incoming raw data to be received, archived, processed and curated. For more info, contact the [Data Engineering](https://asdslack.slack.com/messages/C8X3PP1TN) team.

You can see data sources in the control panel, for the ones that you have permissions to access.
...
### Path specific access{#path-specific-access}
...
4.  Either leave the 'Paths' field blank or enter a list of paths to provide restricted access, as described in \@ref(path-specific-access).
...
3.  Either leave the 'Paths' field blank or enter a list of paths to provide restricted access, as described in \@ref(path-specific-access).
4.  Select __Save__.
